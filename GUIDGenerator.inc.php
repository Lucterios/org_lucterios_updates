...
// --- Last modification: Date 30 August 2008 16:35:41 By  ---
...
function uuidDecode($uuid) {
	...
	if( is_array($u) && ( count($u) == 5)) {
		...
		$new_sum = hexdec( sprintf('%04x',$new_sum));
		if($new_sum == $sum)$check = 1;
		else $check = 0;
		$ip = clientIPFromHex($u[1]);
		$rez = Array('sum' => $sum,'ip' => $ip,'unixtime' => hexdec($u[2]),'micro' => hexdec($u[3])/65536,'rand' => hexdec($u[4]),'check' => $check);
	}
	return $rez;
}
