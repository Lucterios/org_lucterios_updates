...
$param_val=$params->getParameters('CORE');
$guid=$param_val['GUID'];

$ext = new DBObj_CORE_extension;
$ext->extensionId='CORE';
$ext->find();
$ext->fetch();
...
if(($Rep!==false) && (count($Rep)>0))
{
	...
	foreach($Extension_childs as $Extension_child)
	{
		...
		exec('git --version',$gitval,$gitret);
		if ((($famille!="applis") || ($ext->Appli==$module)) && !is_dir($dir.'/.git') &&  (($module!='SDK') || ($gitret==0) ))
		{
